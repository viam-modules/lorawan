...
func (r *rak7391) parseDataUplink(ctx context.Context, phyPayload []byte, packetTime time.Time, snr float64, sf int, c concentrator) (
	...
	device, err := matchDeviceAddr(devAddrBE, r.devices)
	if err != nil {
	...
	requests := r.getFOptsToSend(fopts, device)
	...
	if fCntUp <= device.FCntUp && device.FCntUp != math.MaxUint16 {
		r.logger.Debugf("skipping uplink message - packet already parsed")
	...
	if downlinkPayload != nil || len(requests) > 0 || sendAck || setDutyCycle {
		if len(device.NwkSKey) == 0 || device.FCntDown == math.MaxUint16 {
			...
			if err = r.sendDownlink(ctx, payload, false, packetTime, c); err != nil {
				return "", map[string]interface{}{}, fmt.Errorf("failed to send downlink: %w", err)
			}
			r.logger.Debugf("sent a downlink to %s", device.NodeName)
		}
	}
...
func (r *rak7391) getFOptsToSend(fopts []byte, device *node.Node) []byte {
	...
	for _, b := range fopts {
		...
		default:
			...
			r.logger.Debugf("got unsupported mac command %x from %s", b, device.NodeName)
		}
	}
	return requests
}
