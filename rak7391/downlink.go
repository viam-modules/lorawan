...
func (r *rak7391) sendDownlink(ctx context.Context, payload []byte, isJoinAccept bool, packetTime time.Time, c *concentrator) error {
	...
	txPkt := &lorahw.TxPacket{
		Freq:      r.regionInfo.Rx2Freq,
		DataRate:  rx2SF,
		Bandwidth: r.regionInfo.Rx2Bandwidth,
...
func (r *rak7391) createDownlink(ctx context.Context,
	...
	if len(uplinkFopts) != 0 {
		for _, b := range uplinkFopts {
			switch b {
			case deviceTimeCID:
				r.logger.Debugf("got device time request from %s", device.NodeName)
				deviceTimeAns, err := createDeviceTimeAns()
				if err != nil {
					r.logger.Errorf("failed to create device time answer: %w", err)
				}
				fopts = append(fopts, deviceTimeAns...)
			case linkCheckCID:
				r.logger.Debugf("got link check request from %s", device.NodeName)
				linkCheckAns := createLinkCheckAns(snr, sf)
				fopts = append(fopts, linkCheckAns...)
			case dutyCycleCID:
				r.logger.Debugf("got duty cycle answer from %s", device.NodeName)
			default:
	...
	if device.Region == regions.EU && device.FCntDown == 0 {
		...
		r.logger.Debugf("sending duty cycle request to %s", device.NodeName)
	}
