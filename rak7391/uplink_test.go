...
func TestParseDataUplink(t *testing.T) {
	...
	deviceName, readings, err := g.parseDataUplink(context.Background(), testRxPacket, time.Now(), c)
	test.That(t, err, test.ShouldBeNil)
	...
	test.That(t, err, test.ShouldBeNil)
	testRxPacket.Payload = invalidPayload

	_, _, err = g.parseDataUplink(context.Background(), testRxPacket, time.Now(), c)
	...
	test.That(t, err, test.ShouldBeNil)
	testRxPacket.Payload = unknownPayload
	...
	test.That(t, err, test.ShouldBeNil)
	invalidMICPayload[len(invalidPayload)-1] = 0x00
	test.That(t, err, test.ShouldBeNil)
	testRxPacket.Payload = invalidMICPayload
	...
	test.That(t, err, test.ShouldBeNil)
	testRxPacket.Payload = validPayload
	...
	testRxPacket.Payload = []byte{0x00, 0x00}
	_, _, err = g.parseDataUplink(context.Background(), testRxPacket, time.Now(), c)
	test.That(t, err, test.ShouldNotBeNil)
	...
	test.That(t, err, test.ShouldBeNil)
	testRxPacket.Payload = noFramePayload
	_, _, err = g.parseDataUplink(context.Background(), testRxPacket, time.Now(), c)
	test.That(t, err, test.ShouldNotBeNil)
