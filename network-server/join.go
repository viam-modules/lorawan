...
func (g *NetworkServer) handleJoin(ctx context.Context, payload []byte, packetTime time.Time, gateway sensor.Sensor) error {
	...
	if err != nil {
		return fmt.Errorf("error sending downlink: %w", err)
	}
	return nil
...
func (g *NetworkServer) parseJoinRequestPacket(payload []byte) (joinRequest, *node.Node, error) {
...
func (g *NetworkServer) generateJoinAccept(ctx context.Context, jr joinRequest, d *node.Node) ([]byte, error) {
